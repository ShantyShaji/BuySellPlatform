...
const AdsDetails = () => {
  ...
  useEffect(() => {
    const fetchAdDetails = async () => {
      try {
        const response = await axios.get(`https://ads.planetmedia.app/api/advertisements/${id}`, {
          headers: {
            'x-api-key': apiKey, // Use API key from AuthContext
          },
        });
        setAdDetails(response.data); // Store the fetched ad details
      } catch (err) {
  ...
  const { title, description, image, price, owner } = adDetails;
    ...
    <div className="w-full min-h-screen flex flex-col items-center justify-center">
      ...
      <div className="lg:w-[86vw] flex-1 p-6 mt-20">
        ...
        <div className="lg:flex lg:justify-between">
          {/* Overview */}
          <div className="bg-white rounded-xl shadow p-6 lg:w-[70%]">
            <h1 className="text-xl lg:text-2xl font-semibold">{title || 'No Title Available'}</h1>
            <p className="text-gray-500 mt-1 flex items-center gap-2">
              <IoLocationOutline className="text-[#F50963]" /> Location not available
              <SlCalender className="text-[#F50963] ml-3" /> Date not available
            </p>
          ...
          <div className="flex flex-col gap-5 mt-6 lg:mt-0 lg:w-[28%]">
            <div className="bg-white rounded-xl shadow p-6 flex justify-between items-center border border-gray-100 mb-4">
              <div className="flex flex-col gap-1">
                <span className="font-bold text-gray-700">Price:</span>
                <span className="text-pink-600 text-3xl font-bold">${price || 'N/A'}</span>
              </div>
              <img src="/tag.png" alt="Price Tag" className="h-10" />
            </div>
            <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center border border-gray-100">
              <div className="mt-6 flex flex-col items-center">
                <img
                ...
                <p className="font-medium text-gray-800">
                  {owner?.firstName || 'Unknown'} {owner?.lastName || 'Seller'}
                </p>
                <p className="text-gray-500 text-sm">Member since N/A</p>
              </div>
              ...
              <div className="mt-6 space-y-3 w-[80%]">
                <button className="w-full flex items-center justify-start gap-4 py-2 px-4 rounded-md text-white bg-stone-100">
                  ...
                  <div className="flex flex-col items-start justify-center">
                    <p className="text-gray-500 text-sm">Click to Show Number</p>
                    <p className="text-sm text-black font-semibold">+88xXXXXXXXXX</p>
                  </div>
                </button>
                <button className="w-full flex items-center justify-start gap-4 py-2 px-4 rounded-md text-white bg-stone-100">
                  <div className="text-[#F50963] p-3 bg-white shadow-lg rounded-full">
                  ...
                  <div className="flex flex-col items-start justify-center">
                    <p className="text-gray-500 text-sm">Click to Show Email</p>
                    <p className="text-sm text-black font-semibold">infoXXXXXXXXX</p>
                  </div>
                </button>
              </div>
            </div>
